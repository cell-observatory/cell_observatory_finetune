...
from segmentation.models.utils.model_utils import DropPath, trunc_normal_, PatchEmbed
from segmentation.models.utils.model_utils import MLPBlock as Mlp
...
def window_partition(x, window_size):
    ...
    if len(x_shape) == 5:
        b, d, h, w, c = x_shape
        # (b, window_num_d, window_elem_d, window_num_h, window_elem_h, window_num_w, window_elem_w, c)
        x = x.view(
            b,
            d // window_size[0],
            window_size[0],
        ...
        # (b*window_num_d*window_num_h*window_num_w, window_elem_d*window_elem_h*window_elem_w, c)
        windows = (
...
def window_reverse(windows, window_size, dims):
    ...
    if len(dims) == 4:
        ...
        # (b,window_num_d,window_elem_d,window_num_h,window_elem_h,window_num_w,window_elem_w,c)->(b,d,h,w,c)
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        if len(self.window_size) == 3:
            # as K = (2D - 1) * (2H - 1) * (2W - 1) = total nr of distinct rel pos in 3D
            # implicitly we let bias be defined for 
            # index = Δz * ((2H - 1) * (2W - 1)) + Δy * (2W - 1) + Δx
            self.relative_position_bias_table = nn.Parameter(
                torch.zeros(
                    (2 * self.window_size[0] - 1) * (2 * self.window_size[1] - 1) * (2 * self.window_size[2] - 1),
                    num_heads, # 1 table for each head
                )
            ) 
            ...
            # compute pairwise relative coords. 
            relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]
            relative_coords = relative_coords.permute(1, 2, 0).contiguous() # (N, N, 3)
            
            # shift to start from 0 (coords now in [0, 2D-2], [0, 2H-2], [0, 2W-2])
            relative_coords[:, :, 0] += self.window_size[0] - 1
            relative_coords[:, :, 1] += self.window_size[1] - 1
            relative_coords[:, :, 2] += self.window_size[2] - 1
            
            # 3D bias grid => flattened 1D grid
        ...
        # set flat_index = z * (2H-1)(2W-1) + y * (2W-1) + x
    ...
    def forward(self, x, mask):
        ...
        q = q * self.scale
        attn = q @ k.transpose(-2, -1) # (3,b,num_heads,n,n)
        
        # rel_pos_index is a flat index of the flat relative position bias table
        # which we reshape to (n, n, num_heads) 
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        if len(x_shape) == 5:
            b, d, h, w, c = x.shape
            # ensure window_size <= axis_size else set to max and set shift_size=0
            window_size, shift_size = get_window_size((d, h, w), self.window_size, self.shift_size)
            
            pad_l = pad_t = pad_d0 = 0
            pad_d1 = (window_size[0] - d % window_size[0]) % window_size[0] # need to pad d axis s.t. multiple of window_size[0]
        ...
        # patchify, compute attention, and reverse patchification
