...
from segmentation.layers.layers import Conv3d, MLP, inverse_sigmoid
from segmentation.structures.data_objects.boxes import box_xyzxyz_to_cxcyczwhd, masks_to_boxes, masks_to_boxes_v2
from segmentation.models.heads.decoders.dino_decoder import TransformerDecoder, DeformableTransformerDecoderLayer
...
class MaskDINODecoder(nn.Module):
    ...
    def gen_encoder_output_proposals(memory, memory_padding_mask, shapes):
        ...
        for lvl, (D, H, W) in enumerate(shapes):
            ...
            # NOTE: torch.expand does not allocate new memory, it just creates a view
            #       but only works if the size of the tensor is 1 in that dimension whereas torch.repeat 
            #       allocates new memory but can be used in any dimension
    ...
    def denoise_post_process(self, outputs_labels, outputs_bboxes, outputs_mask, denoise_data):
        ...
        out = {'pred_logits': outputs_labels_denoise[-1], 'pred_boxes': outputs_bboxes_denoise[-1], 'pred_masks': outputs_mask_denoise[-1]}
