...
def paste_mask_in_image(mask, box, im_d, im_h, im_w):
    # type: (Tensor, Tensor, int, int, int) -> Tensor
    TO_REMOVE = 1
...
class RoIHeads(nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                match_quality_matrix = box_ops.box_iou(gt_boxes_in_image, proposals_in_image) # iou similarity matrix
    ...
    def forward(
        ...
        proposals,  # type: List[Tensor]
        image_shapes,  # type: List[Tuple[int, int, int]]
        ...
        Args:
            ...
            proposals (List[Tensor[N, 6]])
            image_shapes (List[Tuple[D, H, W]])
            targets (List[Dict])
        """
        ...
        if self.has_mask():
            ...
            # mask_proposals = [self.filter_boxes(p) for p in mask_proposals] #TODO: Is this needed?
...
############################################################ LEGACY ############################################################
    ...
    #     keep = (widths > eps) & (heights > eps) & (depths > eps)
    #     return boxes[keep]
