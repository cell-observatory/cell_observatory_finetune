...
def restore_model(config: DictConfig):
    ...
    try: # check if model already exists
        ...
        loss_record = training_history[f"val_{config.metrics.ckpt_loss_key}"]
        best_loss = loss_record.min()
...
def get_optimizer(params, config: DictConfig, optimizer: str, steps_per_epoch: int, deepspeed_scheduler: bool = False):
    ...
    if deepspeed_scheduler:
        ...
        logger.info(
            f"Training [epochs: {config.schedulers.epochs}, steps_per_epoch: {steps_per_epoch} = total_steps: {total_steps}, "
            f"warmup: {config.schedulers.warmup} = warmup_steps: {warmup_steps}, decay_epochs: {decay_epochs},"
            f"decay_steps: {decay_steps}]"
        )
...
def supervised(config: DictConfig):
    ...
    val_steps_per_epoch = int(np.ceil(len(val_dataloader) / (config.clusters.total_gpus))) if val_dataloader else None
    ...
    with torch.autograd.set_detect_anomaly(True, check_nan=False):
        with torch.profiler.profile(
            ...
            with_flops=True,
            with_modules=True,
        ) if config.deepspeed.profile else nullcontext() as profiler:
            for epoch in range(starting_epoch, config.schedulers.epochs):
                ...
                for step, data_sample in enumerate(train_dataloader):
                    step_time = time.time()
                    lr_value = opt.param_groups[0]["lr"]
                    
                    # TODO: refactor decoder loss computation
                    loss_dict, outputs = model(data_sample)
                        ...
                        with torch.no_grad():
                            ...
                            for val_step, (inputs, targets) in enumerate(val_dataloader):
                                ...
                                if val_step % config.val_log_step == 0:
                                    ...
                                    logger.info(f"│ Epoch (val): {epoch+1}/{config.schedulers.epochs}")
                            ...
                            if ckpt_loss < best_loss:
                                ...
                                if config.deepspeed.zero_optimization.stage == 3:
